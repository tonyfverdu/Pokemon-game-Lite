...
/*
A medida que nuestra aplicación crece, queremos dividirla en múltiples archivos, llamados “módulos”. 

Un módulo puede contener una "clase" o una "biblioteca de funciones" para un propósito específico.

Durante mucho tiempo, JavaScript existió sin una sintaxis de módulo a nivel de lenguaje. Eso no fue un problema, porque inicialmente los scripts eran pequeños 
y simples, por lo que no era necesario. Pero con el tiempo los scripts se volvieron cada vez más complejos, por lo que la comunidad inventó una variedad de formas 
de organizar el código en "módulos", "bibliotecas especiales" para cargar módulos a pedido.
...
El "sistema de módulos" a nivel de idioma apareció en el estándar en 2015, evolucionó gradualmente desde entonces y ahora es compatible con todos los principales 
navegadores y en Node.js. Así que estudiaremos los módulos modernos de Javascript de ahora en adelante.


1.-  //  Qué es un módulo?

Un "módulo" es solo "un archivo". Un script es un módulo. Tan sencillo como eso. Los "módulos" pueden cargarse entre sí y usar directivas especiales "export" e "import" 
para intercambiar funcionalidad, llamar a funciones de un módulo de otro:

    A.-  Palabra clave "export":  La palabra clave "export" etiqueta las variables y funciones que deberían ser accesibles desde fuera del módulo actual.

    B.-  Palabra clave "import":  La palabera clave "import" permite importar funcionalidades desde otros módulos.
...
La directiva "import" carga el módulo "sayHi" por la ruta relativa: "./sayHi.js" relativo con el archivo actual, y asigna la función exportada "sayHi" a la 
variable correspondiente.

Ejecutemos el ejemplo en el navegador.

Los "módulos" admiten palabras clave y características especiales, debemos decirle al navegador que un "script" debe tratarse como un "módulo", utilizando 
el atributo <script type =" module ">.
...
Si intenta abrir una página web localmente a través del protocolo file://, encontrará que las directivas "import" y "export" no funcionan. Use un servidor web local, 
como "static-server" o use la capacidad de “servidor en vivo” de su editor, como VS Code Live Server Extension para probar los módulos.


2.-  Características del "módulo central".

¿Qué hay de diferente en los "módulos" en comparación con los scripts “normales”? Hay características principales, válidas tanto para el navegador como para 
JavaScript del lado del servidor.


3.-  Siempre en modo estricto.
...
Los "módulos" deben hacer "export" a lo que ellos quieren que esté accesible desde afuera y hacer "import" de lo que necesiten.
    ...
    "hello.js" debe "importar" la desde el módulo "user.js".
...
Así todos los scripts la verán, con o sin type="module". Dicho esto, hacer este tipo de variables globales está muy mal visto. Por favor evítalas.
